...
io.on('connection', (socket) => {
  ...
  socket.on('createRoom', ({ roomId, username }) => {
    ...
    logger.info(`Room ${roomId} created by ${username}`)
  })
  ...
  socket.on('joinRoom', ({ roomId, username }) => {
    ...
    const isUsernameTaken = room.players.some(player => player.username.toLowerCase() === username.toLowerCase())
  ...
  socket.on('toggleReady', ({ roomId }) => {
    ...
    logger.info(`Player ${player.username} ${player.ready ? 'is now ready' : 'is no longer ready'} in room ${roomId}`)
  })
  ...
  socket.on('updateGameSettings', ({ roomId, settings }) => {
    ...
    logger.info(`Game settings updated in room ${roomId}`)
  })
  ...
  socket.on('playerGuess', ({ roomId, guessResult }) => {
    ...
    logger.info(
      `Player ${player.username} made a guess in room ${roomId}: ${guessResult.name} (${
  ...
  socket.on('gameEnd', ({ roomId, result }) => {
    ...
    const allEnded = room.players.every(
      p => p.guesses.includes('✌') || p.guesses.includes('💀') || p.guesses.includes('🏳️') || p.disconnected,
    )
    ...
    logger.info(`Player ${player.username} ended their game in room ${roomId} with result: ${result}`)
  })
  ...
  socket.on('requestGameSettings', ({ roomId }) => {
    ...
    if (room.settings) {
      ...
      logger.info(`Game settings sent to new player in room ${roomId}`)
    }
  })
  ...
  socket.on('surrender', ({ roomId }) => {
    ...
    logger.info(`Player ${player.username} surrendered in room ${roomId}`)
  })
  ...
  socket.on('timeOut', ({ roomId }) => {
    ...
    logger.info(`Player ${player.username} timed out in room ${roomId}`)
  })
  ...
  socket.on('disconnect', () => {
    ...
    for (const [roomId, room] of rooms.entries()) {
      ...
      if (playerIndex !== -1) {
        ...
        if (room.host === socket.id) {
          ...
          logger.info(`Host ${disconnectedPlayer.username} disconnected. Room ${roomId} closed and disbanded.`)
        }
        else {
          ...
          logger.info(
            `Player ${disconnectedPlayer.username} ${
              disconnectedPlayer.score === 0 ? 'removed from' : 'disconnected from'
            } room ${roomId}.`,
          )
        }
    ...
    logger.info(`User ${socket.id} disconnected`) // General disconnect log
  })
  ...
  socket.on('toggleRoomVisibility', ({ roomId }) => {
    ...
    logger.info(`Room ${roomId} visibility changed to ${room.isPublic ? 'public' : 'private'}`)
  })
})
